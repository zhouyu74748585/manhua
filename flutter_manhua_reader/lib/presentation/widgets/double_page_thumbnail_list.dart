...
class DoublePageThumbnailList extends StatefulWidget {
  ...
  @override
  State<DoublePageThumbnailList> createState() => _DoublePageThumbnailListState();
}
...
class _DoublePageThumbnailListState extends State<DoublePageThumbnailList> {
  ...
  void didUpdateWidget(DoublePageThumbnailList oldWidget) {
    ...
    if (oldWidget.currentGroupIndex != widget.currentGroupIndex && _isInitialized) {
      _previewIndex = widget.currentGroupIndex;
  ...
  void _updateVisibleItems() {
    ...
    // 计算可见范围
    final startIndex = (scrollOffset / itemTotalWidth).floor().clamp(0, widget.doublePageGroups.length - 1);
    final endIndex = ((scrollOffset + viewportWidth) / itemTotalWidth).ceil().clamp(0, widget.doublePageGroups.length);
    ...
    // 预加载前后的图片
    final preloadStart = (startIndex - widget.preloadCount).clamp(0, widget.doublePageGroups.length);
    final preloadEnd = (endIndex + widget.preloadCount).clamp(0, widget.doublePageGroups.length);
  ...
  void _scrollToCurrentGroup() {
    ...
    final targetOffset = (widget.currentGroupIndex * itemTotalWidth) - viewportCenter + (widget.itemWidth / 2);
              ...
              return GestureDetector(
                ...
                child: AnimatedContainer(
                  ...
                  decoration: BoxDecoration(
                    border: Border.all(
                      color: isPreviewGroup ? Colors.blue : (isCurrentGroup ? Colors.amber : Colors.transparent),
  ...
  Widget _buildSingleThumbnail(int pageIndex) {
    ...
    if (page.largeThumbnail != null && File(page.largeThumbnail!).existsSync()) {
      return Image.file(
        ...
        errorBuilder: (context, error, stackTrace) => _buildThumbnailPlaceholder(pageIndex + 1),
      );
      ...
      return CachedNetworkImage(
        ...
        width: double.infinity,
        height: double.infinity,
        placeholder: (context, url) => _buildThumbnailPlaceholder(pageIndex + 1),
        errorWidget: (context, url, error) => _buildThumbnailPlaceholder(pageIndex + 1),
      );
    } else {
      return _buildThumbnailPlaceholder(pageIndex + 1);
    }
