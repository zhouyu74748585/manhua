...
class StorageService {
  ...
  static List<Map<String, dynamic>> getLibraries() {
    final libraries = _storage[AppConstants.librariesKey] ?? <Map<String, dynamic>>[];
    return List<Map<String, dynamic>>.from(libraries);
  }
  
  // 阅读进度
  static Future<void> setReadingProgress(String mangaId, Map<String, dynamic> progress) async {
  ...
  static Map<String, int> getStorageInfo() {
    final cacheCount = _storage.keys.where((key) => key.startsWith('cache_')).length;
    return {
